...
@app.route('/convertScoring', methods=['GET', 'POST'])
def Scoring():
    if request.method == 'GET':
        return render_template('ScoreClick.html')
    else:
        result = {}
...
@app.route('/rootSquare', methods=['GET', 'POST'])
def RootSquare():
    if request.method == 'GET':
        return render_template('rootScore.html')
    else:
        result = {}
        data = request.get_json()
        if data.get('number', ''):
